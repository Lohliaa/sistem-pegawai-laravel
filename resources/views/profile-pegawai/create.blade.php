...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h3 class="mb-0"><i class="bi bi-plus-circle"></i> Tambah Pegawai</h3>
...
<form action="{{ route('profile-pegawai.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="status_pernikahan" id="status_pernikahan" class="form-select">
                                <option value="">-- Pilih --</option>
                                <option value="Lajang" @selected(old('status_pernikahan') === 'Lajang')>Lajang</option>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="status_kepegawaian_id" id="status_kepegawaian_id" class="form-select">
                                ...
                                @foreach($statusList as $st)
                                    <option value="{{ $st->id }}" @selected(old('status_kepegawaian_id') == $st->id)>{{ $st->nama_status }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label for="tanggal_tmt" class="form-label">Tanggal TMT</label>
                            <input type="date" name="tanggal_tmt" id="tanggal_tmt" class="form-control" value="{{ old('tanggal_tmt') }}">
                        </div>
                        <div class="col-md-6">
                            <label for="golongan_ruang" class="form-label">Golongan/Ruang</label>
                            <input type="text" name="golongan_ruang" id="golongan_ruang" class="form-control" value="{{ old('golongan_ruang') }}">
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="status_aktif" id="status_aktif" class="form-select">
                                @foreach(['Aktif', 'Resign', 'Pensiun', 'PHK', 'Meninggal', 'Pensiun Dini'] as $statusOption)
                                    <option value="{{ $statusOption }}" @selected(old('status_aktif') === $statusOption)>{{ $statusOption }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label for="nomor_sk" class="form-label">Nomor SK</label>
                            <input type="text" name="nomor_sk" id="nomor_sk" class="form-control" value="{{ old('nomor_sk') }}">
                        </div>
                        <div class="col-md-6">
                            <label for="tanggal_sk" class="form-label">Tanggal SK</label>
                            <input type="date" name="tanggal_sk" id="tanggal_sk" class="form-control" value="{{ old('tanggal_sk') }}">
                        </div>
                        <div class="col-md-6">
                            <label for="masa_kerja" class="form-label">Masa Kerja</label>
                            <input type="text" name="masa_kerja" id="masa_kerja" class="form-control" value="{{ old('masa_kerja') }}" placeholder="Misal: 5 Tahun 2 Bulan">
                        </div>
                        <div class="col-md-6">
                            ...
                            <select name="user_id" id="user_id" class="form-select">
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" @selected(old('user_id') == $user->id)>{{ $user->username }} ({{ strtoupper($user->role) }})</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="pendidikan_terakhir" id="pendidikan_terakhir" class="form-control" value="{{ old('pendidikan_terakhir') }}" placeholder="Contoh: S1, S2">
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="institusi" id="institusi" class="form-control" value="{{ old('institusi') }}">
                        </div>
                        <div class="col-md-6">
                            <label for="tahun_lulus" class="form-label">Tahun Lulus</label>
                            <input type="text" name="tahun_lulus" id="tahun_lulus" class="form-control" value="{{ old('tahun_lulus') }}" maxlength="4">
                        </div>
                        <div class="col-md-12">
                            <label for="sertifikasi" class="form-label">Sertifikasi</label>
                            <textarea name="sertifikasi" id="sertifikasi" rows="3" class="form-control">{{ old('sertifikasi') }}</textarea>
                        </div>
                        <div class="col-md-12">
                            <label for="pelatihan" class="form-label">Pelatihan</label>
                            <textarea name="pelatihan" id="pelatihan" rows="3" class="form-control">{{ old('pelatihan') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="col-md-12">
                            ...
                            <select name="data_bpi" id="data_bpi" class="form-select">
                                ...
                                @foreach(['Jenjang Dasar', 'Menengah', 'Lanjutan'] as $bpiOption)
                                    <option value="{{ $bpiOption }}" @selected(old('data_bpi') === $bpiOption)>{{ $bpiOption }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-12">
                            <label for="data_presensi" class="form-label">Data Presensi</label>
                            <textarea name="data_presensi" id="data_presensi" rows="2" class="form-control">{{ old('data_presensi') }}</textarea>
                        </div>
                        <div class="col-md-12">
                            <label for="data_cuti" class="form-label">Data Cuti</label>
                            <textarea name="data_cuti" id="data_cuti" rows="2" class="form-control">{{ old('data_cuti') }}</textarea>
                        </div>
                        <div class="col-md-12">
                            <label for="riwayat_jabatan" class="form-label">Riwayat Jabatan</label>
                            <textarea name="riwayat_jabatan" id="riwayat_jabatan" rows="2" class="form-control">{{ old('riwayat_jabatan') }}</textarea>
                        </div>
                        <div class="col-md-12">
                            <label for="riwayat_mutasi" class="form-label">Riwayat Mutasi</label>
                            <textarea name="riwayat_mutasi" id="riwayat_mutasi" rows="2" class="form-control">{{ old('riwayat_mutasi') }}</textarea>
                        </div>
                        <div class="col-md-12">
                            <label for="riwayat_status_kepegawaian" class="form-label">Riwayat Status Kepegawaian</label>
                            <textarea name="riwayat_status_kepegawaian" id="riwayat_status_kepegawaian" rows="2" class="form-control">{{ old('riwayat_status_kepegawaian') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="mt-4 pb-5">
        <button type="submit" class="btn btn-primary btn-lg"><i class="bi bi-save"></i> Simpan Data Pegawai</button>
